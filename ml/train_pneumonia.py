...
class PneumoniaClassifier:
    ...
    def create_data_loaders(self, data_path):
        """Create data loaders for pneumonia training and testing"""
        train_dir = os.path.join(data_path, "pneumonia", "train")
        test_dir = os.path.join(data_path, "pneumonia", "val")
    ...
    def plot_training_history(self, train_losses, val_losses, train_accs, val_accs):
        ...
        ax1.set_title("Pneumonia - Model Loss")
        ...
        ax2.set_title("Pneumonia - Model Accuracy")
...
def main():
    """Main function to run pneumonia training"""
    classifier = PneumoniaClassifier(image_size=224, batch_size=32)
    model, test_loader = classifier.train_model(epochs=20)
    print("\nPneumonia detection model trained successfully!")
